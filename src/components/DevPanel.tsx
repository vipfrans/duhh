...
import { getStock, clearStock, setStock } from "@/lib/usernames";
...
export default function DevPanel({ onStockChange }: DevPanelProps) {
  ...
  // Load current stock into textarea when panel opens
  useEffect(() => {
    if (phase === "panel") {
  ...
  // Focus key input when prompt opens
  useEffect(() => {
    if (phase === "key") {
  ...
  function handleApply() {
    ...
    setStock(lines);
    onStockChange();
    setApplyMsg({ ok: true, msg: `✓ ${lines.length} usernames applied to stock` });
            ...
            >
              ...
              <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column", padding: "16px 20px", gap: "10px" }}>
                <p style={{ fontSize: "12px", color: "#475569" }}>
                  One username per line. Press <strong style={{ color: "#94a3b8" }}>Apply</strong> to save to stock.
                </p>
                <textarea
                  ref={textareaRef}
                  value={text}

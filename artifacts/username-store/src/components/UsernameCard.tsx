...
export default function UsernameCard() {
  ...
  // Load stock from server on mount + poll every 10s for real-time sync
  useEffect(() => {
    let cancelled = false;
    const refresh = async () => {
      const count = await getStockCount();
      if (!cancelled) setStock(count);
    };
    refresh();
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "28px" }}>
        <div style={{
          ...
          boxShadow: "0 0 20px rgba(168,85,247,0.4)", flexShrink: 0,
        }}>4L</div>
        <div>
          <h2 style={{
          ...
          <p style={{ fontSize: "12px", color: "#64748b", marginTop: "2px" }}>4-letter username drops</p>
        </div>
      ...
      <div style={{
        ...
        }}>{stock.toLocaleString()}</span>
      </div>

      {/* Claimed username */}
      {claimed && (
        <div style={{
          background: showSuccess ? "rgba(16,185,129,0.08)" : "rgba(255,255,255,0.03)",
          border: showSuccess ? "1px solid rgba(16,185,129,0.25)" : "1px solid rgba(255,255,255,0.06)",
          ...
          <div>
            ...
            <span style={{
              fontFamily: "'JetBrains Mono', monospace",
              fontSize: "clamp(18px, 3.5vw, 22px)", fontWeight: 600,
              color: showSuccess ? "#10b981" : "#e2e8f0", letterSpacing: "0.08em",
            }}>@{claimed}</span>
          </div>
          <button
            ...
            style={{
              ...
              borderRadius: "8px", padding: "8px 10px", color: "#64748b",
      ...
      {cooldown > 0 && (
        <div style={{ marginBottom: "16px" }}>
          <div style={{ height: "3px", background: "rgba(255,255,255,0.06)", borderRadius: "2px", overflow: "hidden" }}>
            <div style={{
              height: "100%", width: `${cooldownPct}%`,
              background: "linear-gradient(90deg, #a855f7, #3b82f6)", borderRadius: "2px",
              transition: "width 1s linear", boxShadow: "0 0 8px rgba(168,85,247,0.6)",
            }} />
          </div>
        </div>
      )}
      ...
      <button
        ...
        disabled={isDisabled}
        style={{
          width: "100%", padding: "clamp(12px, 2.5vw, 15px) 24px",
          ...
          ...(isDisabled
            ? { background: "rgba(255,255,255,0.04)", color: "#475569", border: "1px solid rgba(255,255,255,0.06)", boxShadow: "none" }
            : { background: "linear-gradient(135deg, #a855f7, #3b82f6)", color: "#fff", boxShadow: "0 0 30px rgba(168,85,247,0.35), 0 4px 20px rgba(0,0,0,0.3)" }),
        }}
        onMouseEnter={(e) => {
          if (!isDisabled) { e.currentTarget.style.transform = "translateY(-1px)"; e.currentTarget.style.boxShadow = "0 0 50px rgba(168,85,247,0.5), 0 8px 30px rgba(0,0,0,0.4)"; }
        }}
        onMouseLeave={(e) => {
          if (!isDisabled) { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "0 0 30px rgba(168,85,247,0.35), 0 4px 20px rgba(0,0,0,0.3)"; }
        }}
      >
        {isLoading ? (
